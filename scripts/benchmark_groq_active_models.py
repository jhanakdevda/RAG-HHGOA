"""
Phase 4: Groq Active Models Latency & Quality Benchmark Script
"""
...
def benchmark_active_model(model_name: str, target_successes: int = 3) -> Dict[str, Any]:
    print("-" * 90, flush=True)
    print(f"BENCHMARKING ACTIVE GROQ MODEL: {model_name}", flush=True)
    print("-" * 90, flush=True)
...
def run_groq_comparison():
    ...
    print("=" * 90, flush=True)
    print("ACTIVE GROQ MODELS BENCHMARK COMPARISON", flush=True)
    print("=" * 90, flush=True)

    results = []
    for model in candidate_models:
        res = benchmark_active_model(model, target_successes=3)
        results.append(res)
        time.sleep(8.0)

    print("\n" + "=" * 90, flush=True)
    print("FINAL GROQ ACTIVE MODELS BENCHMARK COMPARISON TABLE", flush=True)
    print("=" * 90, flush=True)
